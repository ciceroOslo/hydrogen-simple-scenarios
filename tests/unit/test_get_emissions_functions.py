...
def test_soToHydrogenEmissionsConverter():
    ...
    assert converter_1.get_co_to_h2_factor_burning_cmip6("Grassland Burning") == converter_1.get_co_to_h2_factor_burning_cmip6("Savanna")
    assert converter_1.get_co_to_h2_factor_burning_cmip6("Forest Burning")  == converter_1.get_co_to_h2_factor_burning_cmip6("Extratropical Forest")
    assert converter_1.get_co_to_h2_factor_burning_cmip6("Typo") == converter_1.get_co_to_h2_factor_burning_cmip6("Garbage Burning")
    assert converter_3.get_co_to_h2_factor_burning_cmip6("Agricultural Waste Burning") == 0   
    ...
    assert converter_2.get_co_to_h2_factor("1A4c") == converter_1.get_co_to_h2_factor_cmip6("Random")
    assert converter_1.get_co_to_h2_factor("7C") == converter_2.get_co_to_h2_factor_cmip6("Energy sector")
    assert converter_2.get_co_to_h2_factor_cmip6("Agricultural Waste Burning") == 0
    assert converter_1.get_co_to_h2_factor_cmip6("Agricultural Waste Burning") == 2.59 / 102
...
def test_get_sector_column():
    ...
    assert sorted(sector_column.columns) == sorted(["H2", "CO2", "CO", "CH4", "NMVOC", "NOx"])
    assert(sector_column.shape) == (1, 6)
    print(sector_column)
    sector_column2 = get_emissions_functions.get_sector_column("natural_gas", type_split="fuel")
    assert sorted(sector_column2.columns) == sorted(["H2", "CO2", "CO", "CH4", "NMVOC", "NOx"])
    assert np.all(sector_column2.values < sector_column.values)

    sector_column3 = get_emissions_functions.get_sector_column_total("natural_gas", type_split="fuel")
