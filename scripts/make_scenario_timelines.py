...
from hydrogen_simple_scenarios import (
    ...
    single_year_numbers_check,
    scenario_info,
)

years = range(2020, 2051)
smooth_timeline_blue = CubicSpline(
    scenario_info.dnv_scenario_timeline[0],
    np.array(scenario_info.dnv_scenario_timeline[1]) * 100,
)
smooth_timeline_green = CubicSpline(
    scenario_info.dnv_scenario_timeline[0],
    np.array(scenario_info.dnv_scenario_timeline[2]) * 100,
)
...
# Make table S2
df_to_make_dnv_table = pd.DataFrame(
    data={
        "Blue H2 (Tg/yr)": blue_hydrogen_timeline,
        "Green H2 (Tg/yr)": green_hydrogen_timeline,
    },
    index=years,
)
...
fig1, axs1 = plt.subplots(
    ncols=len(leak_rates) + 1, nrows=1, sharey=True, figsize=(16, 8)
)
fig1.suptitle("Hydrogen production and use breakdown", fontsize=size)
axs1[0].stackplot(
    years,
    blue_hydrogen_timeline,
    green_hydrogen_timeline,
    colors=["tab:blue", "tab:green"],
    labels=["Fossil-based with CCS", "Renewable electrolysis"],
    edgecolor="white",
)
...
sectors = {
    "Feedstock from blue": [
        get_emissions_functions.get_sector_column_total(
            scenario_info.sector_info["current_hydrogen"][0]
        ),
        feedstock_total_TG,
    ...
    "Natural gas": [
        get_emissions_functions.get_sector_column_total(
            scenario_info.sector_info["natural_gas"][0]
        ),
        ng_total_TG,
        scenario_info.blue_pes,
    ],
    ...
    "Steel": [
        get_emissions_functions.get_sector_column_total(
            scenario_info.sector_info["steel"][0]
        ),
        steel_total_TG,
        scenario_info.green,
    ],
    "Generic energy": [
        get_emissions_functions.get_sector_column_total(
            scenario_info.sector_info["total"][0]
        ),
        bg_total_TG,
        scenario_info.green,
    ],
...
for j, leak_rate in enumerate(leak_rates):
    blue_avail = timeseries_functions.get_hydrogen_available(
        blue_hydrogen_timeline, leak_rate
    )
    green_avail = timeseries_functions.get_hydrogen_available(
        green_hydrogen_timeline, leak_rate
    )
    ...
    for i, year in enumerate(years):
        ...
        ts_feedstock_green[i] = min(
            1 - ts_feedstock_blue[i], 0.5 * green_avail[i] / feedstock_total_TG
        )
        # Rest of green to saturation of steel
        ts_steel[i] = min(
            (green_avail[i] - ts_feedstock_green[i] * feedstock_total_TG)
            / steel_total_TG,
            1,
        )
        # Rest of blue to saturation of natural gas
        ts_ng[i] = min(
            (blue_avail[i] - ts_feedstock_blue[i] * feedstock_total_TG) / ng_total_TG, 1
        )
        # Whatever remains follows general background energy sector
        ts_bg_energy[i] = (
            green_avail[i]
            - ts_feedstock_green[i] * feedstock_total_TG
            - ts_steel[i] * steel_total_TG
        ) / bg_total_TG + (
            blue_avail[i]
    ...
    axs1[j + 1].stackplot(
        ...
        blue_avail,
        green_avail,
        colors=[to_rgba("tab:blue", 0.2), to_rgba("tab:green", 0.2)],
        edgecolor=(1, 1, 1, 1),
    )
    axs1[j + 1].set_title(
        f"Leak rate {int(leak_rate*100)}%", fontsize=size * 0.9, fontweight="bold"
    )

    # Now for the timeseries of GWP(*) and mitigated warming
    ts_list = [ts_feedstock_blue, ts_ng, ts_feedstock_green, ts_steel, ts_bg_energy]
    gwp_starish = np.zeros((len(ts_list), len(years)))
    gwp_just_co2 = np.zeros((len(ts_list), len(years)))
    gwp_other_comps = np.zeros(
        (len(ts_list), len(get_emissions_functions.GWP_dict) - 1, len(years))
    )
    for i, ts in enumerate(ts_list):
        emissions_ts = timeseries_functions.make_emission_ts(
            sectors[sectors_list[i]][0], ts, years
        )
        # print(emissions_ts)
        if leak_rate > 0:
            emissions_ts = timeseries_functions.add_leakage_ts(
                sectors[sectors_list[i]][0],
                ts,
                years,
                leak_rate,
                sectors[sectors_list[i]][1] * 1e3,
            )
        # print(emissions_ts)
        emissions_ts = timeseries_functions.add_prod_emissions_ts(
            emissions_ts,
            sectors[sectors_list[i]][1] * 1e3,
            sectors[sectors_list[i]][2],
            years,
            ts,
        )
        # print(emissions_ts)
        # sys.exit(4)
        gwp_starish[i, :], gwp_just_co2[i, :], gwp_other_comps[i, :, :] = (
            timeseries_functions.calc_gwp_star(emissions_ts, years)
        )
...
secax2 = axs2[1].secondary_yaxis(
    "right", functions=(lambda x: x * 0.4, lambda x: x / 0.4)
)
secax3 = axs3[1].secondary_yaxis(
    "right", functions=(lambda x: x * 0.4, lambda x: x / 0.4)
)
