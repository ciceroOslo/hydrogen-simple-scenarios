    ...
    for scen in ssps:
        ...
        for sector in ssp_data_extraction.co_ssp_sectors_burning:
            ...
            if ssp_burning is None:
                ...
                ssp_burning = ssp_burning + ssp_data_extraction.get_ts_component_sector_region_ssp(
                    ssp_file,
                    scen,
                    comp,
                    sector=sector,
        ...
        ssp_tot_interp = np.append(rcmip_data_total, np.interp(range(2015, 2101), years_ssp, ssp_total))
        print(rcmip_data_total[-1])
        print(ssp_tot_interp[0])
        ssp_noburn_interp = np.append(rcmip_no_burning, np.interp(range(2015, 2101), years_ssp, ssp_no_burning))
        print(ssp_noburn_interp[2015 -1750-5: 2015 -1750+5])
        # print out file
        df_tot = pd.DataFrame({'Years': range(1750, 2101), 'Emis': ssp_tot_interp})
        df_noburn = pd.DataFrame({'Years': range(1750, 2101), 'Emis': ssp_noburn_interp})
