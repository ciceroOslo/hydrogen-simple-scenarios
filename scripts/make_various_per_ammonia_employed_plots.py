...
def make_single_year_per_ammonia_gwp_benefit_comparison(
    ...
    if not just_CO2:
        eqvi_str = " equivalent"
    axs[0].set_ylabel(f"CO2{eqvi_str} benefit per unit NH3")
    if star:
        secax.set_ylabel("Mitigated warming (mK per GT NH3)")
    fig.tight_layout()
    fname_suffix = ""
    if star:
...
plt.tight_layout()
plt.savefig("benefits_loss_nh3.png")
