...
for i, sector in enumerate(scenario_info.sector_info):
    ...
    benefit_loss_to_plot = benefit_loss.T.reindex(
        ["Green", "Blue_optimistic", "Blue_compliance"]
    )
...
annuual_natural_gas_2019_bcm = (
    4e3 * 0.85
)  # 3976.2 billion cubic meters 2019 (Statista https://www.statista.com/statistics/265344/total-global-natural-gas-production-since-1998/)
...
bcm_to_cubic_cms = 1.0e9 * 1e6
annual_methane_per_year_Tg = (
    annuual_natural_gas_2019_bcm * g_per_cubic_cm_methane * bcm_to_cubic_cms / 1e12
)
ng_distr_emissions = 11.6583435344712  # CEDS Tg
ng_prod_emissions = 24.0092315386876  # CEDS Tg
print(
    f"Annual methane prod/use: {annual_methane_per_year_Tg }Tg, leaks in distribution: {ng_distr_emissions/annual_methane_per_year_Tg*100}% "
)
print(
    f"Leaks in production: {ng_prod_emissions/annual_methane_per_year_Tg*100 }%, leaks total: {(ng_distr_emissions+ng_prod_emissions)/annual_methane_per_year_Tg*100}%"
)
